...
// Rendu d'explications pour plusieurs PNJs
func (e *Engine) RenderExplanationPnj2(m entity.Pnj, sentence string) {
    rl.BeginMode2D(e.Camera)
    rl.DrawText(
        sentence,
        int32(m.Position.X)-200, // Décalage de 200 pixels à gauche
        int32(m.Position.Y)+30,   // Position verticale (30 pixels en dessous)
        10,
        rl.Black, // Couleur du texte
    )
    rl.EndMode2D()
}

// Identiques pour Pnj3, Pnj4, Pnj5, avec des légères variations dans le décalage Y
